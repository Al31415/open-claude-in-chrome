#!/usr/bin/env python3
"""Thinking-token analog of ratio_decay.png (time) and turns_ratio_decay.png
(turns). Ratio of an arm's estimated total thinking tokens for a task to the
baseline (cold) total for that same task, log y-axis, vs. baseline thinking
tokens (linear x-axis, in thousands). Same "does the relative cost decay as
tasks get longer" question, now for the dominant share of model inference
(thinking is ~87-90% of output tokens per turn, per latency_thinking.png).
Horizontal line at ratio=1 is break-even vs. cold. Fixed y-ceiling with
top-edge ticks for the handful of outliers, same convention as the other two
ratio-decay charts. Fit is linear regression of ln(ratio) on baseline k-tokens
per series (semi-log fit - straight line here); same pooling as before
(experiential: 2a+3a+4a dashed, expert: 2b+3b+4b dashed, solo: 5b/6a/6b
solid). Reads thinking_per_task.json (run compute_thinking_per_task.py first).
Chrome headless -> PNG."""
...
TASKS = list(TPT["1a"].keys())
BASE_K = {t: (sum(TPT[a][t] for a in ("1a", "1b", "1c")) / 3) / 1000 for t in TASKS}
...
def pooled_xy(arms):
    xs, ys = [], []
    for t, bk in BASE_K.items():
        for arm in arms:
            v = TPT[arm].get(t)
            if v is None: continue
            xs.append(bk); ys.append((v / 1000) / bk)
    return xs, ys
...
s.append(f'<text x="{PL}" y="30" font-size="18" font-weight="700" fill="#1b1f24">Thinking tokens as a multiple of cold: source pooled across phases 2-4, plus 5b, 6a, 6b</text>')
s.append(f'<text x="{PL}" y="50" font-size="12.5" fill="#8a929c">Each point: (arm task thinking tokens) / (baseline cold task thinking tokens), log y-axis, vs. baseline thinking tokens (thousands), linear x-axis</text>')
s.append(f'<text x="{PL}" y="{PT-10}" font-size="10.5" fill="#8a929c">horizontal line = 1&#215; (equal to cold) &#183; dashed = pooled source fit, ln(ratio) &#126; baseline k-tokens (n=36 each) &#183; solid = single-arm fit (n=12) &#183; ticks along the top edge = points above {YHI:g}&#215;, off scale</text>')

for g in range(0, int(xmax) + 1, 2):
    s.append(f'<line x1="{X(g):.1f}" y1="{PT}" x2="{X(g):.1f}" y2="{H-PB}" stroke="#f2f1ed"/>')
    s.append(f'<text x="{X(g):.1f}" y="{H-PB+18}" text-anchor="middle" font-size="10" fill="#8a929c">{g}k</text>')
...
s.append(f'<text x="{(PL+W-PR)/2:.1f}" y="{H-14}" text-anchor="middle" font-size="12" fill="#5b6571">baseline (cold) thinking tokens for the task (thousands)</text>')
s.append(f'<text x="18" y="{(PT+H-PB)/2:.1f}" text-anchor="middle" font-size="12" fill="#5b6571" transform="rotate(-90 18 {(PT+H-PB)/2:.1f})">task thinking tokens / baseline (log scale)</text>')
...
s.append('</svg>')
svg = "\n".join(s)
open(os.path.join(HERE, "thinking_ratio_decay.html"), "w").write(
    f'<!doctype html><meta charset="utf-8"><style>*{{margin:0}}body{{background:#fff}}</style>{svg}')
print("wrote", os.path.join(HERE, "thinking_ratio_decay.html"))

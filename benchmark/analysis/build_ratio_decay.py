#!/usr/bin/env python3
"""Ratio of actual task time to baseline (cold) task time, log y-axis, vs.
baseline time (minutes, linear x-axis). Unlike raw minutes, this ratio is a
genuine decay curve for the recipe/warm-up arms: it starts above 1 on short
tasks (fixed setup cost not yet paid off) and decays below 1 as tasks get
longer (net faster than cold). A single horizontal line at ratio=1 is the
break-even reference, replacing the y=x diagonal from the raw-minutes version.
Fit is linear regression of ln(ratio) on baseline minutes per series (semi-log
fit - straight line here); same pooling as before (experiential: 2a+3a+4a
dashed, expert: 2b+3b+4b dashed, solo: 5b/6a/6b solid). Chrome headless
-> PNG."""
...
def pooled_xy(arms):
    ...
    for t, bmin in BASE_MIN.items():
        for arm in arms:
            ...
            xs.append(bmin); ys.append(v / bmin)
    return xs, ys
...
for side, arms in (("A", A_SIDE), ("B", B_SIDE)):
    ...
    print(f"{side}-side pooled: n={len(xs)}  ln(ratio)~x  slope={b1:+.3f}  R2={r2:.3f}  ratio@min={math.exp(b0+b1*min(xs)):.2f} ratio@max={math.exp(b0+b1*max(xs)):.2f}")
...
for arm in SOLO:
    ...
    print(f"{arm}: n={len(xs)}  slope={b1:+.3f}  R2={r2:.3f}  ratio@min={math.exp(b0+b1*min(xs)):.2f} ratio@max={math.exp(b0+b1*max(xs)):.2f}")
...
xmax = max(allx) * 1.1
# fixed y-axis, shared verbatim across ratio_decay.py / turns_ratio_decay.py /
# thinking_ratio_decay.py so the three charts are visually comparable rather
# than each auto-scaling to its own data
YLO = 0.45
YHI = 2.0
...
s.append(f'<text x="{PL}" y="30" font-size="18" font-weight="700" fill="#1b1f24">Task time as a multiple of cold: source pooled across phases 2-4, plus 5b, 6a, 6b</text>')
s.append(f'<text x="{PL}" y="50" font-size="12.5" fill="#8a929c">Each point: (arm task time) / (baseline cold task time), log y-axis, vs. baseline (cold) task time in minutes, linear x-axis</text>')
s.append(f'<text x="{PL}" y="{PT-10}" font-size="10.5" fill="#8a929c">horizontal line = 1&#215; (equal to cold) &#183; dashed = pooled source fit, ln(ratio) &#126; baseline minutes (n=36 each) &#183; solid = single-arm fit (n=12) &#183; ticks along the top edge = points above {YHI:g}&#215;, off scale</text>')
...
def top_tick(x, col, big):
    r = 5 if big else 4
    op = 0.8 if big else 0.55
    s.append(f'<line x1="{X(x):.1f}" y1="{ey:.1f}" x2="{X(x):.1f}" y2="{ey+9:.1f}" stroke="{col}" stroke-width="{2 if big else 1.6}" opacity="{op}"/>')
...
s.append('</svg>')
svg = "\n".join(s)
open(os.path.join(HERE, "ratio_decay.html"), "w").write(
    f'<!doctype html><meta charset="utf-8"><style>*{{margin:0}}body{{background:#fff}}</style>{svg}')
print("wrote", os.path.join(HERE, "ratio_decay.html"))
